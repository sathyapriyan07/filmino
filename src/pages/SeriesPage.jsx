    ...
    <div className="bg-[#0B0B0F] min-h-screen">
      <div className="ott-container py-10 md:py-14">
        ...
        <div className="flex items-end justify-between mb-8 md:mb-10">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-white tracking-tight text-cinematic">Series</h1>
        ...
        <div className="flex items-center gap-2 mb-6 overflow-x-auto scrollbar-hide pb-1">
          ...
          {SORT_OPTIONS.map(opt => (
            <button
              ...
              className={cn('filter-chip flex-shrink-0', filters.sortBy === opt.value ? 'filter-chip-active' : 'filter-chip-inactive')}
            >
              {opt.label}
            </button>
          ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => setFilter('genre', '')} className={cn('filter-chip', !filters.genre ? 'filter-chip-active' : 'filter-chip-inactive')}>All</button>
                  {genres.map(g => (
                    <button key={g.id} onClick={() => setFilter('genre', filters.genre === g.name ? '' : g.name)}
              ...
              <div className="flex items-center gap-2 mt-5 pt-5 border-t border-white/8 flex-wrap">
                {filters.genre && (
                  <span className="flex items-center gap-1.5 text-xs font-medium bg-indigo-500/15 text-indigo-400 border border-indigo-500/25 rounded-full px-3 py-1">
                    {filters.genre}
                    <button onClick={() => setFilter('genre', '')}><X className="h-3 w-3" /></button>
                  </span>
                )}
                {filters.minRating && (
                  <span className="flex items-center gap-1.5 text-xs font-medium bg-amber-500/15 text-amber-400 border border-amber-500/25 rounded-full px-3 py-1">
                    ≥ {filters.minRating}.0 ★
                    <button onClick={() => setFilter('minRating', '')}><X className="h-3 w-3" /></button>
                  </span>
                )}
                <button onClick={clearFilters} className="text-xs text-white/35 hover:text-white/60 transition-colors ml-auto">Clear all</button>
              </div>
            )}
          </div>
        )}
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3 md:gap-4">
          {loading
            ? Array.from({ length: LIMIT }).map((_, i) => <MovieCardSkeleton key={i} width="w-full" />)
            : series.map(s => <MovieCard key={s.id} item={s} type="series" width="w-full" />)
          }
        </div>
        ...
        <div ref={loaderRef} className="h-16 flex items-center justify-center mt-6">
          ...
          {!loading && !loadingMore && series.length > 0 && series.length >= total && (
            <p className="text-xs text-white/20">All {total.toLocaleString()} series loaded</p>
          )}
        </div>
      </div>
    </div>
